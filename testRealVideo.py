...
def main():
	def process_audio(audio,idx):
		...
		while sliding_window_start + samples_per_window < audio_length:
			...
			#get mouthroi
			frame_index_start = int(round((sliding_window_start + idx-audio_length) / opt.audio_sampling_rate * 25))
		...
		audio_spec = torch.FloatTensor(audio_spec).unsqueeze(0).cuda()
		#get mouthroi
		frame_index_start = int(round((idx - samples_per_window) / opt.audio_sampling_rate * 25)) - 1
	...
	for speaker_index in range(opt.number_of_speakers):
		...
		sr, full_audio = wavfile.read(opt.audio_path)	
		print("sampling rate of audio: ", sr)
		if len((full_audio.shape)) == 2:
			full_audio = np.mean(audio, axis=1) #convert to mono if stereo
		full_audio = full_audio / 32768
		full_audio = full_audio / 2.0
		full_audio = clip_audio(full_audio)
		full_audio_length = len(full_audio)

		step_rate = 10            # steps per second
		step = int(sr/step_rate)  # samples per second
		max_step = 40             # maximum steps to feed into network
		min_step = 10             # minimum steps to feed into network

		audio = full_audio[:2*step]
		last_power = loudness(audio[:step])
		power = loudness(audio[-step:])
		last_decreased = False
		decreased = power-last_power < 0
		step_count = 2

		sep_audio = np.array([])
		for idx in range(2*step,full_audio_length):
			if idx%step == 0:
				step_count += 1
				power = loudness(audio[-step:])
				print(f"{100*idx/full_audio_length:.0f}% (loudness: {power:.4f})")
				decreased = power-last_power < 0
				if (last_decreased and decreased and step_count >= min_step) or step_count >= max_step:
					# pause is detected if loudness decreased in the last two steps
					print(f"\tpause detected (step_count: {step_count}, audio_length: {len(audio)})")
					sep_audio = np.append(sep_audio,process_audio(audio,idx))
					step_count = 0
					audio = np.array([])
				last_power = power
				last_decreased = decreased
			audio = np.append(audio,full_audio[idx])

		#output separated audios
		if not os.path.isdir(opt.output_dir_root):
			os.mkdir(opt.output_dir_root)
		sf.write(os.path.join(opt.output_dir_root, 'speaker' + str(speaker_index+1) + '.wav'), sep_audio, opt.audio_sampling_rate)         
